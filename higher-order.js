...
//Code Here
let myStrongest let myStrongest = monstersInYourPocket.filter(function(monster) {
  // console.log(monster.CP);
  if (monster.CP > 200) {
    return monster;
  }
});
...
let orderTotals = orders.map(function(total) {
  let realTax = total.tax + 1;
  // console.log(total.price * realTax);
  return total.price * realTax;
});
